...
def train_adagp(model, predictor, dataloader, num_epochs=20, warmup_epochs=5, lr=0.001, fixed_output_size=1000, save_dir="./checkpoints", device="cuda", start_epoch=0):
    ...
    if use_checkpoint:
        ...
        checkpoint_path = "/content/checkpoints/checkpoint_epoch_1_Warm-Up.pt"
        ...
        try:
            ...
            if os.path.exists(checkpoint_path):
                ...
                last_epoch, last_phase = load_checkpoint(
                    model, predictor, optimizer_model, optimizer_predictor, scheduler_model, scheduler_predictor, checkpoint_path
                )
            ...
            print(f"Error handling checkpoint: {e}")
            print("Starting training from scratch.")
            start_epoch = 0
    else:
        print("Checkpoint usage is disabled.")
        # Initialize training parameters
        start_epoch = 0
...
vgg.to(device)
test_accuracy = evaluate_model(vgg, testloader, device=device)
